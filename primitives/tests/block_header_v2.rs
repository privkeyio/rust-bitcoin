...
use bitcoin_primitives::block::{Header, HeaderV2, Version};
...
/// `CBlockHeader::GetHash` that reproduces every intermediate value in `VECTORS`. The 160 cases
/// here cross all four ASIC profiles with a zero and a non-zero XOR key, the time offset on and
/// off, and the boundary values of the mask clear-bit count.
...
fn knots_vectors() {
    ...
    for vector in headers {
        ...
        let want = Header {
            // Knots stores the version with the extended-form flag stripped, and so do we.
            ...
            time: BlockTime::from_u32(u32_field(f, "nTime")),
        ...
        let serialized = unhex(vector["serialized"].as_str().expect("serialized"));
        assert_eq!(serialized.len(), Header::V2_SIZE, "{}", name);
        ...
        let got: Header = encoding::decode_from_slice(&serialized).expect(name);
        ...
        let want_hash = vector["block_hash"].as_str().expect("block_hash");
        assert_eq!(got.block_hash().to_string(), want_hash, "{}", name);

        // And it round-trips byte for byte.
        assert_eq!(encoding::encode_to_vec(&got), serialized, "{}", name);
    }
}
...
fn v1_header_unchanged() {
    ...
    let header: Header = encoding::decode_from_slice(&serialized).expect("decodes");
    ...
    assert_eq!(header.time_on_wire(), header.time.to_u32());
    assert_eq!(
        header.block_hash().to_string(),
        "000000000003ba27aa200b1cecaad478d2b00432346c3f1f3986da1afd33e506"
    );
    assert_eq!(encoding::encode_to_vec(&header), serialized);
}

#[test]
fn truncated_extended_header_is_an_error() {
    let json: serde_json::Value = serde_json::from_str(VECTORS).expect("valid JSON");
    let serialized = unhex(json["headers"][0]["serialized"].as_str().expect("serialized"));
    ...
    for len in [0, 1, 3, 4, 79, 80, 81, 163] {
        assert!(
            encoding::decode_from_slice::<Header>(&serialized[..len]).is_err(),
            "{} bytes decoded as a header",
            len
        );
    }
    assert!(encoding::decode_from_slice::<Header>(&serialized).is_ok());
}
...
fn decodes_across_arbitrary_chunk_boundaries() {
    ...
    for vector in json["headers"].as_array().expect("headers") {
        let serialized = unhex(vector["serialized"].as_str().expect("serialized"));
        let want: Header = encoding::decode_from_slice(&serialized).expect("decodes");

        // The length is only known after the version word, so a reader that splits the stream
        // mid-word must still make progress.
        for split in 1..serialized.len() {
            let mut decoder = <Header as Decode>::Decoder::default();
            for chunk in [&serialized[..split], &serialized[split..]] {
                let mut bytes = chunk;
                decoder.push_bytes(&mut bytes).expect("no error");
                assert!(bytes.is_empty(), "decoder left bytes unconsumed at split {}", split);
            }
            assert_eq!(decoder.end().expect("completes"), want, "split {}", split);
        }
    }
}
...
fn differential_vectors() {
    ...
    for (i, vector) in vectors.iter().enumerate() {
        ...
        assert_eq!(encoding::encode_to_vec(&header), serialized, "vector {}", i);
    }
}
